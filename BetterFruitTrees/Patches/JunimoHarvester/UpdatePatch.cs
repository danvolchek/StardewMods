...
namespace BetterFruitTrees.Patches.JunimoHarvester
{
    ...
    internal class UpdatePatch
    {
        ...
        {
            _lastHarvestedItem = Utils.Reflection.GetField<Item>(__instance, "lastItemHarvested").GetValue();
        }
            ...
            GameLocation location)
        {
            ...
            if (__instance.currentLocation != null && !Utils.Reflection.GetProperty<StardewValley.Buildings.JunimoHut>(__instance, "home").GetValue().noHarvest.Value &&
                Utils.IsAdjacentReadyToHarvestFruitTree(__instance.getTileLocation(), __instance.currentLocation) && (flastItemHarvested.GetValue() == null || flastItemHarvested.GetValue() == _lastHarvestedItem))
            {
