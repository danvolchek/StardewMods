...
namespace ChatCommands.ClassReplacements
{
    ...
    /// </summary>
    class ConsoleChatMessage : ChatMessage
    {
        ...
        public void ConsoleDraw(SpriteBatch b, int x, int y)
        {
            ...
            for (int index = 0; index < this.message.Count; ++index)
            {
                if (this.message[index].emojiIndex != -1)
                    b.Draw(ChatBox.emojiTexture, new Vector2((float)(x + (double)num1 + 1.0), (float)(y + (double)num2 - 4.0)), new Rectangle(this.message[index].emojiIndex * 9 % ChatBox.emojiTexture.Width, this.message[index].emojiIndex * 9 / ChatBox.emojiTexture.Width * 9, 9, 9), Color.White * this.alpha, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
                else if (this.message[index].message != null)
                {
                    if (this.message[index].message.Equals(Environment.NewLine))
                    {
                        ...
                        DrawMonoSpacedString(b, font, this.message[index].message, new Vector2(x + num1, y + num2), this.color * this.alpha, 0.0f, Vector2.Zero, 1f, SpriteEffects.None, 0.99f);
                }
        ...
        private static void DrawMonoSpacedString(SpriteBatch b, SpriteFont font, string text, Vector2 position, Color color,
            float rotation, Vector2 origin, float scale, SpriteEffects effects, float layerDepth)
        {
            foreach (char c in text)
            {
                b.DrawString(font, c + "", position + new Vector2((widestCharacter - font.MeasureString(c + "").X) / 2, 0),
